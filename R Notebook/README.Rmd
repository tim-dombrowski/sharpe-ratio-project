...
The first three lines in the setup chunk below will automatically install any R packages that you may be missing. Another observation to make about the code chunk is that it is labeled as 'setup', which is a special name that the R Notebook will recognize and automatically run prior to running any other code chunk. This is useful for loading in packages and setting up other global options that will be used throughout the notebook. 
